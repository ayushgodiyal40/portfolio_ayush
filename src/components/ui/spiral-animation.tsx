...
class AnimationController {
    ...
    constructor(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, dpr: number, size: number, isDarkMode: boolean) {
        this.canvas = canvas
        this.ctx = ctx
        this.dpr = dpr
        this.size = size
    ...
    public render() {
        ...
        // 绘制星星
        ctx.fillStyle = this.isDarkMode ? 'rgba(255, 255, 255, 0.9)' : 'rgba(50, 50, 50, 0.85)'
    ...
    private drawTrail(t1: number) {
        for (let i = 0; i < this.trailLength; i++) {
            ...
            this.ctx.fillStyle = this.isDarkMode ? 'rgba(255, 255, 255, 0.85)' : 'rgba(50, 50, 50, 0.75)'
            this.ctx.lineWidth = sw
...
export function SpiralAnimation({ isDarkMode = true }: { isDarkMode?: boolean }) {
    ...
    useEffect(() => {
        ...
        // 创建动画控制器
        animationRef.current = new AnimationController(canvas, ctx, dpr, size, isDarkMode)
